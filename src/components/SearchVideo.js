...
const SearchMovies = () => {
  ...
  const HundleChange = (e) => {
    ...
    setPageUrl(
      `https://apimovies.fr/api/movies/search?search=${e.target.value}`,
    );
        ...
        <Container>
          ...
          {fetchedData && (
            <PagesList>
              ...
              <span>...</span>
              {fetchedData.prev_page_url && (
                <button
                  onClick={() => {
                    apiEndPoint.current = fetchedData.prev_page_url;
                    setPageUrl(fetchedData.prev_page_url);
                  }}>
                  {fetchedData.current_page - 1}
                </button>
              )}
              <button style={{ background: 'white' }}>
                {fetchedData.current_page}
              </button>
              {fetchedData.next_page_url && (
                <button
                  onClick={() => {
