...
const Currency = () => {
    ...
    const getData = async () => {
      ...
      try {
        let response = await axios.get(
          `https://v6.exchangerate-api.com/v6/${process.env.REACT_APP_EXCHANGERATE_KEY}`,
        );
  ...
  const convertBtn = () => {
    setResultInput(
      (secondSelect.current.value / firstSelect.current.value) * amount,
    );
  };
  if (resultInput) {
          ...
          <select ref={firstSelect}>
            {currencies &&
              Object.keys(currencies).map((el, index) => {
                return (
                  <option key={index} value={currencies[el]}>
                    {el}
                  </option>
                );
              })}
